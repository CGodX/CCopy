...
!define MUI_PAGE_CUSTOMFUNCTION_PRE SkipIfPassive
; 目录页离开时校验：末尾不是产品名则自动追加 \${PRODUCTNAME}
!define MUI_PAGE_CUSTOMFUNCTION_LEAVE DirLeaveEnsureProductDir
!insertmacro MUI_PAGE_DIRECTORY

; 目录页离开回调：保证安装路径以 \${PRODUCTNAME} 结尾
Function DirLeaveEnsureProductDir
  ${GetFileName} "$INSTDIR" $0
  StrCmp "$0" "${PRODUCTNAME}" +3 0
